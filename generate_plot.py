...
os.chdir("/scratch/users/ahirsch1/chromo_scratch")
...
bead_spacing = np.array([17, 17] * 50)
...
plt.legend()
plt.suptitle("lp = " + str(lp) + ", lt = " + str(lt) + ", lower sim bound " + str(break_boundaries[0]) +", upper sim bound: " + str(break_boundaries[1])+ " bead spacing " + str(bead_spacing[1:5]) + " ...",
             fontsize = 10)
plt.title("Simulation: " + str(latest_sim))
plt.tight_layout()
plot_name = "theory_vs_simulation_" + latest_sim.split("/")[1]
plt.savefig("output/" + latest_sim.split("/")[1]+ "/" + plot_name + ".png")
