...
import numpy as np
from chromo.Epigenmark import Epigenmark
from chromo.Polymer import Polymer
from chromo.MCmove import MCmove
...
def main():
    ...
    test_file_name = "sim_input_for_testing_mc_moves.txt"
    num_polymers = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[0]
    length_bead = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=float)[1]
    num_epigenmark = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[2]
    length_box_x = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=float)[3]
    num_bins_x = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[4]
    length_box_y = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=float)[5]
    num_bins_y = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[6]
    length_box_z = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=float)[7]
    num_bins_z = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[8]
    num_save_mc = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[9]
    num_mc_steps = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=int)[10]
    from_file = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=bool)[11]
    output_dir = np.genfromtxt(input_dir + test_file_name, comments='#', dtype=str)[12]
    num_mc_move_types = 1
    ...
    for mcmove_count in range(1, num_mc_move_types + 1):
        if mcmove_count == 1:   # isolate end-pivot move
