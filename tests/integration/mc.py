...
def andy_mc():
    """
    Original simulation, written by Andy. Should be replaced with a cleaner
    version that doesn't rely on reading input files, except for optionally.
    """
    ...
    polymers = []
    prop_file = input_dir / Path("chromo_prop")
    if not prop_file.exists():
        raise OSError(f"Property file does not exist for chromosomes")
    for chromo_count in range(1, num_polymers + 1):
        polymers.append(Polymer(epigenmarks, num_epigenmark, chromo_count,
                               length_bead, from_file, input_dir))
