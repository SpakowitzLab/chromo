            ...
            with open(pdb_path_new, 'w') as f:
                ...
                if file_ind < num_files:
                    for i in range(max_beads_per_file):
                        line = get_PDB_string(r_x[start_ind + i], r_y[start_ind + i], r_z[start_ind + i], i+1, HP1[start_ind + i]+PRC1[start_ind + i])
                        f.write(line + "\n")
                else:
                    for i in range(start_ind, num_beads):
                        line = get_PDB_string(r_x[i], r_y[i], r_z[i], i+1-start_ind, HP1[i]+PRC1[i])
                        f.write(line + "\n")
