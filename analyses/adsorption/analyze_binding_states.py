...
class CommandLineUsage():
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "-s", "--sim",
            help="Simulation snapshot to analyze. "
                "Example: `--sim 1` will load simulation snapshot 1.",
            type=int,
            metavar="",
            required=True
        )
        parser.add_argument(
            "-p", "--protein",
            help="Protein for which to analyze binding state. "
                "Example: `--protein HP1` will load patterns of HP1 binding.",
            metavar="",
            required=True
        )
        parser.add_argument(
            "-l",
            "--lower",
            help="First snapshot index for average binding state. "
                "Example: `--lower 20` averages binding states starting at "
                 "snapshot 20.",
            metavar="",
            type=int,
            required=True
        ...
        parser.add_argument(
            "-u",
            "--upper",
            help="(OPTIONAL) Last snapshot index for average binding state. "
                "Example: `--upper 100` averages binding states up to "
                 "snapshot 100.",
            required=False,
            type=int,
            metavar=""
        ...
        parser.add_argument(
            "-d", "--output_dir",
            help="(OPTIONAL) Directory containing all simulation outputs. "
                "Example: `--output_dir output` specifies that simulation "
        ...
        parser.add_argument(
            "-o", "--seq_output",
            help="(OPTIONAL) File into which average binding patterns will be saved. "
                "Example: `--seq_output avg_hp1.csv` specifies that average "
                 "binding pattern will be saved into file `avg_hp1.csv`.",
            required=False,
            metavar="",
            default="avg_binding_pattern"
        )
