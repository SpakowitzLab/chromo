...
class Polymer:
    ...
    def __init__(self, epigenmark, num_epigenmark, polymer_count, length_bead, from_file, input_dir):
        ...
        self.name = np.genfromtxt(self.input_dir / Path("chromo_prop"), comments='#', dtype=str)[ind0]
        self.num_beads = np.genfromtxt(self.input_dir / Path("chromo_prop"), comments='#', dtype=int)[ind0 + 1]
