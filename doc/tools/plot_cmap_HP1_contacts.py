...
for i, f in enumerate(output_files):
    ...
    r = np.ascontiguousarray(
        pd.read_csv(output_path, usecols=[1, 2, 3, 10], skiprows=[0, 1]).to_numpy()
    )
    if i == 0:
        n_beads = len(r)
...
np.savetxt(output_dir + "/contact_matrix_single_sim_HP1.csv", contacts, delimiter=",")
np.savetxt(output_dir + "/contacts_at_sep_single_sim_HP1.csv", contacts_at_sep, delimiter=",")
...
plt.tight_layout()
plt.savefig(output_dir + "/contact_matrix_single_sim_HP1.png", dpi=600)
